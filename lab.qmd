...
extract_phi_from_fit <- function(fit, K, p) {
  ...
  if (ncol(B) == K * p + 1) B <- B[, -1, drop = FALSE]
...
companion_max_root <- function(Phi) {
  K <- dim(Phi)[1]; p <- dim(Phi)[3]
  F <- matrix(0, nrow = K*p, ncol = K*p)
  F[1:K, 1:(K*p)] <- do.call(cbind, lapply(1:p, \(j) Phi[, , j]))
  if (p > 1) F[(K+1):(K*p), 1:(K*(p-1))] <- diag(K*(p-1))
  max(Mod(eigen(F, only.values = TRUE)$values))
}

Phi_en <- extract_phi_from_fit(fit_en, K = ncol(Y), p = P_LAGS)
max_root <- companion_max_root(Phi_en)
max_root
