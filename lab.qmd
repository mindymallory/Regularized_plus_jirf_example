...
TICKERS <- c(
  "AAPL","MSFT","AMZN",
  "GOOGL","AVGO","META","BRK.B","LLY","WMT","JPM",
  "V","ORCL","MA","XOM","JNJ"
)
...
P_LAGS  <- 5            # VAR order
STRUCT  <- "BasicEN"      # regularization structure (EN elastic net)
GRAN    <- c(500, 50)   # lambda grid: (path length, # grid points)
H_IRF   <- 20           # horizons: 0..20
SHOCK_SD <- 2           # 2-SD joint shocks
ALPHA_GRID <- seq(0.05, .5, by = 0.05)   # 1 = pure lasso, smaller = more ridge
# -----------------------------
# 1) Load data
# -----------------------------
...
mod_en <- constructModel(
  Y,
  p       = P_LAGS,
  struct  = STRUCT,
  gran    = GRAN,          # your lambda grid settings
  h       = 1,
  cv      = "Rolling",
  T1      = T1,
  T2      = T2,
  ...
  verbose = FALSE,
  model.controls = list(
    intercept = TRUE,      # IMPORTANT for price levels
    alpha     = ALPHA_GRID
  )
)

fit_en <- cv.BigVAR(mod_en)
print(fit_en)
SparsityPlot.BigVAR.results(fit_en)
LAMBDA_FIXED <- fit@OptimalLambda
#


# Save model output for later use, since cross validation takes a long time

saved_model <- list(
  created_utc = as.character(Sys.time()),
  tickers     = colnames(Y),
  p_lags      = P_LAGS,
  struct      = STRUCT,              # e.g. "BasicEN"
  gran        = GRAN,
  T1          = T1,
  T2          = T2,
  ...
  # Optimal tuning
  optimal_lambda = as.numeric(fit_en@OptimalLambda)[1],
  optimal_alpha  = if ("OptimalAlpha" %in% slotNames(fit_en)) as.numeric(fit_en@OptimalAlpha)[1] else NA_real_,

  # What you need for sparsity + IRF
  betaPred = fit_en@betaPred,
  resids   = fit_en@resids
)

dir.create("models", showWarnings = FALSE)
saveRDS(saved_model, file = "models/bigvar_opt_model.rds")
...
saved_model <- readRDS("models/bigvar_opt_model.rds")

B <- as.matrix(saved_model$betaPred)
E <- as.matrix(saved_model$resids)

# Now you can do SparsityPlot/jIRF using B and E (no CV)
...
Phi_hat <- extract_phi_from_fit(fit_en, K = K, p = P_LAGS)
...
MED <- apply(boot_paths, c(1, 2), quantile, probs = 0.5, na.rm = TRUE)
